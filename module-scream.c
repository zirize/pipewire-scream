...
#define MODULE_NAME "module-scream-sender"
...
static void on_stream_process(void *userdata)
{
    ...
    /* Split into Scream packets (max 1152 bytes per packet) */
    uint32_t offset = 0;
    while (offset < size) {
        uint32_t chunk_size = (size - offset > SCREAM_MAX_PAYLOAD) ? 
                              SCREAM_MAX_PAYLOAD : (size - offset);
...
static int init_network(struct scream_sink_data *data)
{
    ...
    if (IN_MULTICAST(ntohl(data->dest_addr.sin_addr.s_addr))) {
        ...
        if (data->interface) {
            ...
            iface_addr.s_addr = inet_addr(data->interface);
...
int pipewire__module_init(struct pw_impl_module *module, const char *args)
{
    ...
    data->port = str ? atoi(str) : SCREAM_DEFAULT_PORT;
    ...
    data->format.rate = str ? atoi(str) : 48000;
    
    str = pw_properties_get(props, "channels");
    data->format.channels = str ? atoi(str) : 2;
